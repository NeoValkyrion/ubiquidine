...
DWORD WINAPI CMainWindow::ProcessThread()
{
	...
	string s1 ="photo.JPG";
	Mat img_object = imread( s1, CV_LOAD_IMAGE_GRAYSCALE );
	int minHessian = 500;
    ...
    while (continueProcessing)
    {
        ...
        if (m_frameHelper.IsInitialized()) 
        {
            ...
            if (!m_bIsColorPaused && SUCCEEDED(m_frameHelper.UpdateColorFrame())) 
            {
				...
				detector.detect( m_colorMat, keypoints_scene );
				extractor.compute( m_colorMat, keypoints_scene, descriptors_scene );
				...
				good_matches.reserve(keypoints_scene.size() + keypoints_object.size());
				...
				if(good_matches.size() > 0) H = findHomography( obj, scene, CV_RANSAC );

				 drawMatches( img_object, keypoints_object, m_colorMat, keypoints_scene,
				...
				if(good_matches.size() > 0)	perspectiveTransform( obj_corners, scene_corners, H);
